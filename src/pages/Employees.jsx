...
export default function Employees() {
  ...
  const handleOpenEdit = (emp) => {
    setEditingId(emp.id);
    setFormData({
      name: emp.name || "", cpf: emp.cpf || "", pis: emp.pis || "",
      role: emp.role || "", status: emp.status || "Ativo",
      salary: emp.salary || "", va: emp.va || "", vr: emp.vr || "", vt: emp.vt || "",
      unitId: emp.unit?.id || "", admissionDate: emp.admissionDate || ""
    });
    setIsModalOpen(true);
  };
  ...
  const handleExport = (statusFilterToExport) => {
      ...
      let csvContent = "Nome,CPF,PIS/NIS,Cargo,Status,Admissao,Loja,Salario,VA,VR,VT\n";
      toExport.forEach(emp => {
          const row = [
              emp.name, emp.cpf, emp.pis, emp.role, emp.status,
              formatTableDate(emp.admissionDate), emp.unit?.name || "Sem Loja",
              emp.salary, emp.va, emp.vr, emp.vt
          ].map(val => `"${val || ''}"`).join(","); 
          csvContent += row + "\n";
      });
      ...
      <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={() => handleExport('Ativo')} className="flex items-center gap-2 bg-white border border-slate-200 text-blue-600 hover:bg-blue-50 px-4 py-2.5 rounded-lg font-bold shadow-sm transition-all text-sm">
                <Download className="w-4 h-4" /> Exportar Ativos
            </button>
            <button onClick={() => handleExport('Desligado')} className="flex items-center gap-2 bg-white border border-slate-200 text-red-600 hover:bg-red-50 px-4 py-2.5 rounded-lg font-bold shadow-sm transition-all text-sm">
                <Download className="w-4 h-4" /> Exportar Desligados
            </button>
            
            <div className="h-8 w-px bg-slate-200 hidden sm:block mx-1"></div>

            <button onClick={() => navigate('/import-employees')} className="flex items-center gap-2 bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 px-4 py-2.5 rounded-lg font-bold shadow-sm transition-all text-sm">
                <Upload className="w-4 h-4" /> Importar
            </button>

            <button onClick={handleOpenCreate} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-bold shadow-sm transition-all text-sm">
                            ...
                            <td className="p-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-[150px]">
                                        <div className="font-bold text-slate-800 text-base line-clamp-1">{emp.name || 'Sem Nome'}</div>
                            ...
                            <td className="p-4">
                                {emp.unit?.name ? (
                                    <div className="flex items-center gap-2 text-slate-700 font-medium">
                                        ...
                                        <span className="line-clamp-2">{emp.unit.name}</span>
                                    </div>
                                ) : (
